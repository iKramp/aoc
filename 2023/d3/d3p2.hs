...
getSymbolIndexRows a = filterReturnIndex a (== '*') 0
...
getGearRatios :: [[Char]] -> [(Int, Int)] -> [Int]
getGearRatios grid = map (getGearRatio grid)

getGearRatio :: [[Char]] -> (Int, Int) -> Int
getGearRatio a (ix, iy) =
    let numIndexes = filterNumIndexes (getNumIndexes 0 a) [(ix, iy)]
        nums = map toNum $ getNums a numIndexes
    in 
        if length nums == 2
            then product nums
            else 0
...
solveGrid a =
    ...
    in sum . getGearRatios a $ symbolCoords
